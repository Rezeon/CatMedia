...
export default function ChatBox({ receiver }) {
    ...
    const fetchMessages = async () => {
        ...
        try {
            const response = await axios.get(route("message.index", { receiverId: receiver.id }));
    ...
    const handleSend = async () => {
        ...
        if (editingMessageId) {
            ...
            try {
                await axios.post(route("message.update", editingMessageId), formData, {
            ...
            if (photoFile instanceof File) {
                formData.append("photo", photoFile); // 💡 pastikan photoFile adalah File
            }
        ...
        <div className="w-full h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto bg-gray-100 p-2 mb-2">
                {messages.map((msg, index) => {
                    ...
                    const prevDate = index > 0 ? dayjs(messages[index - 1].created_at).format("YYYY-MM-DD") : null;
                    const showDateHeader = currentDate !== prevDate;
                    const isOwnMessage = msg.is_own || msg.sender_id !== receiver.id;

                    return (
                        <div key={msg.id}>
                            {showDateHeader && (
                                <div className="text-center text-sm text-gray-500 my-2">
                                    {dayjs(msg.created_at).format("D MMMM YYYY")}
                                </div>
                            )}
                                ...
                                <div className="max-w-[80%] relative group">
                                    <div className="flex flex-col px-3 py-2 bg-white rounded shadow">
                                        <span className="text-xs text-gray-400">
                                            {isOwnMessage ? "You" : receiver.name}
                                        </span>
                                        ...
                                        <div className="text-[10px] text-gray-500 mt-1 text-right">
                                            ...
                                            {msg.updated_at !== msg.created_at && (
                                                <span className="text-red-400 ml-2">Edited</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="flex gap-2 border-t pt-2 mt-2 bg-white p-2">
                <input
                    ...
                    onChange={(e) =>
                        editingMessageId ? setEditingContent(e.target.value) : setMessage(e.target.value)
                    }
